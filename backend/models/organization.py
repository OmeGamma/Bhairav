...
from . import create_organization_document, serialize_doc
...
def create_organization(data: Dict[str, Any]) -> Dict[str, Any]:
    doc = create_organization_document(data)
    collection = get_organizations_collection()
    result = collection.insert_one(doc)
    doc["_id"] = str(result.inserted_id)
    return doc

def get_organizations_by_case(case_id: str) -> List[Dict[str, Any]]:
    collection = get_organizations_collection()
